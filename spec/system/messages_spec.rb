...
RSpec.describe 'メッセージ投稿機能', type: :system do
  ...
  context '投稿に失敗したとき' do
    it '送る値が空の為、メッセージの送信に失敗すること' do
      ...
      # DBに保存されていないことを確認する
      # メッセージを送信するために、findメソッドを使用して、送信ボタンの「input[name="commit"]」要素を取得して、クリック.
      # しかし、何も入力を行っていないので、データベースのmessageモデルのカウントが増えていないことを確認している。
      expect {
        find('input[name="commit"]').click
      }.not_to change { Message.count }

      # 送信に失敗した場合、元のページ（メッセージ一覧画面）に戻ってくることを確認する
